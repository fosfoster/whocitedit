...
def main() -> int:
    ...
    try:
        ...
        bad += check("fields" in passed and "field_works" in passed,
                     "render.main() still calls render_home with the legacy three "
                     f"arguments: it passed {sorted(passed)}")
        if "fields" not in passed or "field_works" not in passed:
            return 1

        fields_index, field_works = passed["fields"], passed["field_works"]
